...
public class Program
{
    ...
    private static void AddEvent(string command)
    {
        ...
        GetEventParameters(command, "AddEvent", out date, out title, out location);
        events.AddEvent(date, title, location);
    }

    private static void GetEventParameters(
    ...
    public class EventHolder
    {
        ...
        public void DeleteEvents(string titleToDelete)
        {
            ...
            int countOfRemovedEvents = 0;

            foreach (var eventToRemove in eventsByTitle[title])
            {
                countOfRemovedEvents++;
                eventsByDate.Remove(eventToRemove);
            }

            eventsByTitle.Remove(title);
            Messages.EventDeleted(countOfRemovedEvents);
        }

        public void ListEvents(DateTime date, int count)
        {
            Event startingEvent = new Event(date, "", "");
            OrderedBag<Event>.View eventsToDisplay = eventsByDate.RangeFrom(startingEvent, true);
            int countOfDisplayedEvents = 0;

            foreach (var eventToDisplay in eventsToDisplay)
            {
                if (countOfDisplayedEvents == count)
                {
                    break;
                }

                Messages.PrintEvent(eventToDisplay);
                countOfDisplayedEvents++;
            }

            if (countOfDisplayedEvents == 0)
            {
